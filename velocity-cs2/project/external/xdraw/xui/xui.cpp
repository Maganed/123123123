...
namespace xui {
	...
	void end_child( )
	{
		...
		if ( win && win->group_id != null_id )
		{
			...
			if ( win->scrollable )
			{
				...
				constexpr auto k_scrollbar_w{ 4.0f };
				constexpr auto k_scrollbar_pad{ 4.0f };
				...
				const auto dt = xdraw::delta_time( );
				const auto sb_speed = sb_target > cs.scrollbar_anim ? 9.0f : 9.0f;
				cs.scrollbar_anim += ( sb_target - cs.scrollbar_anim ) * std::min( sb_speed * dt, 1.0f );

				const auto sb_alpha = ease::out_cubic( cs.scrollbar_anim );

				const auto track = rect
				{
					std::floorf( win->bounds.right( ) - k_scrollbar_w - k_scrollbar_pad ),
				...
				const auto thumb_h = std::floorf( std::max( 24.0f, thumb_h_unclamped ) );
				...
				{
					if ( popup_hovered && input.mouse_clicked && thumb.contains( input.mouse_x, input.mouse_y ) )
					{
					...
					else if ( popup_hovered && input.mouse_clicked && track.contains( input.mouse_x, input.mouse_y ) )
					{
				...
				if ( max_scroll > 0.0f )
				{
					...
					if ( top_t > 0.01f )
					{
						...
						dl.rect_filled_gradient( win->bounds.x, win->bounds.y, win->bounds.w, fade_h, top, top, bg_clear, bg_clear, xdraw::corner_radius{ cr, cr, 0.0f, 0.0f } );
					}
					...
					if ( bot_t > 0.01f )
					{
						...
						dl.rect_filled_gradient( win->bounds.x, win->bounds.bottom( ) - fade_h, win->bounds.w, fade_h, bg_clear, bg_clear, bot, bot, xdraw::corner_radius{ 0.0f, 0.0f, cr, cr } );
					}
				}

				if ( max_scroll > 0.0f && ( sb_alpha > 0.005f || cs.scrollbar_dragging ) )
				{
					const auto display_alpha = sb_alpha;
					auto& dl = draw::current( );
					...
					auto track_col = xdraw::color{ 255, 255, 255, 18 };
					track_col.a = static_cast< std::uint8_t >( track_col.a * display_alpha );

					auto thumb_col = thumb_active ? tokens::col_accent : xdraw::color{ 220, 215, 245, 140 };
					thumb_col.a = static_cast< std::uint8_t >( thumb_col.a * display_alpha );
	...
	namespace {
			...
			float alpha_mult = 1.0f )
		{
			...
			auto border_col = st.checkbox_border;
			if ( border_col.a == 0 )
			{
				border_col = xdraw::color{ 72, 65, 106, 220 };
			}
			border_col.a = static_cast< std::uint8_t >( border_col.a * alpha_mult * ( 1.0f - t * 0.4f ) );
			if ( border_col.a > 0 )
			{
				dl.rect( x, y, size, size, border_col, xdraw::corner_radius{ st.checkbox_rounding }, 1.0f );
			}
			...
			if ( t > 0.01f )
			{
				...
				auto icon_col = st.checkbox_mark_icon;
				icon_col.a = static_cast< std::uint8_t >( 255.0f * t * alpha_mult );
				const auto p1_x = x + size * 0.26f;
				const auto p1_y = y + size * 0.52f;
				const auto p2_x = x + size * 0.44f;
				const auto p2_y = y + size * 0.72f;
				const auto p3_x = x + size * 0.76f;
				const auto p3_y = y + size * 0.30f;
				dl.line( p1_x, p1_y, p2_x, p2_y, icon_col, 1.8f );
				dl.line( p2_x, p2_y, p3_x, p3_y, icon_col, 1.8f );
			}
		}
			...
			const style& st )
		{
			...
			dl.rect( track_x, track_y, track_w, track_h, xdraw::color{ 255, 255, 255, 18 }, xdraw::corner_radius{ track_r }, 1.0f );

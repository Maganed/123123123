...
done_scanning:
		...
		if (result) {
			logging::console::print (xs ("[pattern] [OK]    {} -> {:#x}"), pattern.substr(0, (std::min)(pattern.length(), 40ull)), result);
		} else {
			logging::console::print (xs ("[pattern] [ERROR] {} -> resolution failed"), pattern.substr(0, (std::min)(pattern.length(), 40ull)));
		}

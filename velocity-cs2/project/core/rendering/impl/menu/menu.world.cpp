...
namespace rendering {
	...
	void menu::draw_world( float group_w ) const
	{
		...
		const auto subtab = std::max( 0, this->m_subtab - 3 );
		...
		if ( subtab == 0 )
		{
			...
			const auto left_c1_h = ( total_h - gap ) * 0.72f;
			const auto left_c2_h = total_h - gap - left_c1_h;

			xui::layout::set_cursor( content_x - wx, body_y - wy );

			if ( xui::begin_child( "##esp_items", col_w, left_c1_h, true ) )
			{
			...
			xui::layout::set_cursor( content_x - wx, ( body_y + left_c1_h + gap ) - wy );

			if ( xui::begin_child( "##esp_other", col_w, left_c2_h, true ) )
			{
			...
			if ( xui::begin_child( "##esp_projectiles", col_w, total_h, true ) )
			{
		...
		if ( subtab == 1 )
		{
			...
			if ( xui::begin_child( "##world_scene_left", col_w, total_h, true ) )
			{
			...
			if ( xui::begin_child( "##world_scene_right", col_w, total_h, true ) )
			{
		...
		if ( subtab == 2 )
		{
			...
			if ( xui::begin_child( "##world_weather", col_w, total_h, true ) )
			{

...
namespace rendering {
	...
	void menu::draw_player( float group_w ) const
	{
		...
		const auto col_w = ( this->m_body_w - gap ) * 0.5f;
		...
		if ( has_overlay )
		{
			...
			const auto left_c1_h = ( total_h - gap ) * 0.68f;
			const auto left_c2_h = total_h - gap - left_c1_h;

			xui::layout::set_cursor( this->m_body_x - this->m_x, this->m_body_y - this->m_y );

			if ( xui::begin_child( "##player_esp", col_w, left_c1_h, true ) )
			{
				...
				xui::layout::set_cursor( this->m_body_x - this->m_x, ( this->m_body_y + left_c1_h + gap ) - this->m_y );

				if ( xui::begin_child( "##player_glow", col_w, left_c2_h, true ) )
				{
		...
		else
		{
			...
			if ( xui::begin_child( "##local_chams_glow", col_w, total_h, true ) )
			{
		...
		xui::layout::set_cursor( this->m_body_x - this->m_x + col_w + gap, this->m_body_y - this->m_y );
		...
		if ( has_overlay )
		{
			...
			if ( xui::begin_child( "##player_chams", col_w, total_h, true ) )
			{
		...
		{
			if ( xui::begin_child( "##viewmodel", col_w, total_h, true ) )
			{

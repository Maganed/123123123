...
namespace features::combat {
	...
	bool shared::lagcomp::record::setup( std::uintptr_t pawn )
	{
		...
		{
			static int s_fail_node = 0;
			if ( ++s_fail_node % 128 == 1 )
				logging::console::print( xs( "[lagcomp] setup FAIL: game_scene_node null (pawn={:#x})" ), pawn );
			return false;
		}
		...
		if ( !this->bone_cache )
		{
			static int s_fail_cache = 0;
			if ( ++s_fail_cache % 128 == 1 )
				logging::console::print( xs( "[lagcomp] setup FAIL: bone_cache null (node={:#x}, modelState_off={:#x})" ), this->game_scene_node, SCHEMA( "CSkeletonInstance", "m_modelState"_hash ) );
			return false;
		}
		...
		if ( this->bone_count <= 0 )
		{
			...
			if ( ++s_fail_count % 128 == 1 )
				logging::console::print( xs( "[lagcomp] setup FAIL: bone_count={} (node={:#x})" ), this->bone_count, this->game_scene_node );
			return false;
		}
		...
		if ( !this->bone_cache )
		{
			static int s_fail_cache2 = 0;
			if ( ++s_fail_cache2 % 128 == 1 )
				logging::console::print( xs( "[lagcomp] setup FAIL: bone_cache null after re-evaluation (node={:#x})" ), this->game_scene_node );
			return false;
		}
	...
	void shared::lagcomp::run( )
	{
...

		{
			...
			if ( ++s_log_tick % 128 == 1 )
				logging::console::print( xs( "[lagcomp] run: players={}, pending={}, setup ok={}, fail={}, total_records={}" ), active.size( ), pending.size( ), setup_ok, setup_fail, this->m_records.size( ) );
		}
	...
	void shared::update( )
	{
		...
		if ( !local.pawn )
		{
			static int s_fail_pawn = 0;
			if ( ++s_fail_pawn % 128 == 1 )
				logging::console::print( xs( "[shared] update FAIL: no pawn" ) );
			return;
		}
		...
		{
			static int s_fail_gvars = 0;
			if ( ++s_fail_gvars % 128 == 1 )
				logging::console::print( xs( "[shared] update FAIL: global_vars={:#x}, movement_services={:#x} (off={:#x})" ), global_vars, movement_services, SCHEMA( "C_BasePlayerPawn", "m_pMovementServices"_hash ) );
			return;
		}
		...
		{
			static int s_fail_ws = 0;
			if ( ++s_fail_ws % 128 == 1 )
				logging::console::print( xs( "[shared] update FAIL: weapon_services null (pawn={:#x}, off={:#x})" ), local.pawn, SCHEMA( "C_BasePlayerPawn", "m_pWeaponServices"_hash ) );
			return;
		}
		...
		if ( !weapon_handle )
		{
			static int s_fail_handle = 0;
			if ( ++s_fail_handle % 128 == 1 )
				logging::console::print( xs( "[shared] update FAIL: m_hActiveWeapon handle=0 (ws={:#x}, off={:#x})" ), this->m_ctx.weapon_services, SCHEMA( "CPlayer_WeaponServices", "m_hActiveWeapon"_hash ) );
			return;
		}
		...
		{
			static int s_fail_lookup = 0;
			if ( ++s_fail_lookup % 128 == 1 )
				logging::console::print( xs( "[shared] update FAIL: entity lookup failed (handle={:#x})" ), weapon_handle );
			return;
		}
		...
		{
			static int s_fail_vdata = 0;
			if ( ++s_fail_vdata % 128 == 1 )
				logging::console::print( xs( "[shared] update FAIL: weapon_vdata null (weapon={:#x}, subclass_off={:#x})" ), this->m_ctx.weapon, SCHEMA( "C_BaseEntity", "m_nSubclassID"_hash ) );
			return;
		}
	...
	math::vector3 shared::find_spread_correction( const math::vector3& aim_angle, int tick, const math::vector3& aim_punch ) const
	{
		const auto total_spread = this->m_ctx.inaccuracy + this->m_ctx.spread;
		if ( total_spread < 0.0001f )
		{
			return aim_angle;
		}

		// Probe across pitch angles to find a self-consistent fixed point where:
		// get_spread_seed( cmd_angle, tick ) == seed used to compute spread compensation.
		// When an exact match is found, the server's spread calculation is 100% neutralized.
		for ( auto i = 0; i < 1440; ++i )
		{
			const auto probe_pitch = ( static_cast< float >( i ) * 0.25f ) - 89.0f;
			const auto test_angles = math::vector3{
				probe_pitch,
			...
			const auto spread = this->calculate_spread(
				seed,
				this->m_ctx.inaccuracy,
				this->m_ctx.spread,
				this->m_ctx.recoil_index,
				this->m_ctx.item_def_idx,
				this->m_ctx.num_bullets
			);
			...
			adj_angle.x = std::clamp( adj_angle.x, -89.0f, 89.0f );
			adj_angle.y = math::helpers::normalize_yaw( adj_angle.y );
			adj_angle.z = math::helpers::normalize_yaw( adj_angle.z );

			// Check if the command angles as received and processed by the server hash to the exact same seed
			const auto cmd_angles = math::vector3{
				std::clamp( adj_angle.x - aim_punch.x, -89.0f, 89.0f ),
				math::helpers::normalize_yaw( adj_angle.y - aim_punch.y ),
				adj_angle.z
			};

			if ( this->get_spread_seed( cmd_angles, tick ) == seed )
			{
				return adj_angle;
			}
		}

		// If no exact match exists on this tick, return zero vector so ragebot skips this tick
		// and fires on the next tick with guaranteed precision instead of missing with random roll!
		return {};
	}
	...
	bool shared::can_shoot( systems::input::usercmd* cmd, std::uintptr_t local_controller, bool check_next_attack ) const
	{
		...
		const auto client_tick = ( base_cmd && base_cmd->client_tick( ) > 0 ) ? base_cmd->client_tick( ) : tick_base;
		const auto current_tick = std::max( client_tick, tick_base );
		...
		if ( this->m_ctx.weapon_type == cstypes::weapon_type::knife )
		{
			...
			return ( current_tick >= next_primary || current_tick >= next_secondary );
		}

		return current_tick >= next_primary;
	}
	...
	bool shared::is_max_accuracy( float inaccuracy ) const
	{
		...
		if ( on_ground )
		{
			const auto accurate_threshold = this->m_ctx.weapon_max_speed * 0.34f;
			if ( speed > accurate_threshold )
			{
				return false;
			}
			...
			if ( this->m_ctx.weapon_type == cstypes::weapon_type::sniper )
			{
				...
				if ( this->m_ctx.weapon_vdata )
				{
					const auto base_inaccuracy = is_ducking
						? memory::read<float>( this->m_ctx.weapon_vdata + SCHEMA( "CCSWeaponBaseVData", "m_flInaccuracyCrouch"_hash ) )
						: memory::read<float>( this->m_ctx.weapon_vdata + SCHEMA( "CCSWeaponBaseVData", "m_flInaccuracyStand"_hash ) );

					return inaccuracy <= base_inaccuracy + 0.003f || speed <= 1.0f;
				}
			}

...
namespace features::combat {
	...
	void rage::on_create_move( systems::input::usercmd* cmd )
	{
		...
		if ( !ctx.valid )
		{
			static int s_skip_invalid = 0;
			if ( ++s_skip_invalid % 128 == 1 )
				logging::console::print( xs( "[rage] SKIP: shared ctx invalid (see [shared] logs)" ) );
		...
		else
		{
			...
			const auto can_fire = g_shared.can_shoot( cmd, local.controller );
			this->run_gun( cmd, aim_ctx, local, can_fire );
		}
	}
	...
	std::vector<rage::candidate> rage::gather_candidates( const systems::local::snapshot& local, float max_distance_sq ) const
	{
		...
		const_cast<rage*>( this )->m_extrapolated_records.reserve( std::max( players.size( ) * 2, std::size_t{ 64 } ) );
		...
		for ( const auto& p : players )
		{
			...
			if ( memory::read<bool>( pawn + SCHEMA( "C_CSPlayerPawn", "m_bIsDefusing"_hash ) ) )
			{
			}
			...
			else if ( settings::g_combat.m_lagcomp.extrapolation.value )
			{
				auto extrap = g_shared.lc( ).extrapolate( pawn );
				if ( extrap.has_value( ) )
				{
	...
	void rage::run_gun( systems::input::usercmd* cmd, const aim_context& ctx, const systems::local::snapshot& local, bool allow_fire )
	{
		...
		const auto standing_inaccuracy = ( duckpeek_active || ctx.on_ground ) ? this->get_standing_inaccuracy( local, ctx ) : ctx.predicted_inaccuracy;
		const auto standing_hc = best.valid
			? ( ( duckpeek_active || ctx.on_ground ) ? this->evaluate_hitchance( best.hit, ctx, standing_inaccuracy ) : best.hitchance )
			: 0.0f;
		...
		// predicted stopped eye or 1-tick peek extrapolation; airborne stopping keeps the current target context.
		if ( autostop_enabled && !shot_viable && this->should_stop_movement( ctx ) )
		{
			...
			if ( stop )
			{
				...
				{
					const auto peek_offset = prestate.networked_velocity * cstypes::tick_interval;
					auto peek_hits = scan_from_eye_candidates( peek_offset, ctx.predicted_inaccuracy );
					const auto peek_target = this->select_best( ctx, peek_hits, ctx.predicted_inaccuracy );
					if ( peek_target.valid )
					{
			...
			{
				features::combat::g_misc.autostop( ).on_create_move( cmd );
			}
		}
	...
	{
		auto hitgroup_priority = [ ]( int hitbox_index, bool prioritize_body ) -> int
			{
				if ( prioritize_body )
				{
					// If target is low HP or lethal, prioritize large, reliable hitboxes (stomach / pelvis / chest) over head
					if ( hitbox_index >= 2 && hitbox_index <= 3 ) { return 5; } // stomach / pelvis
					if ( hitbox_index >= 4 && hitbox_index <= 6 ) { return 4; } // chest
					if ( hitbox_index == 0 ) { return 3; }                      // head
		...
		auto cheap_score = [ & ]( const scan_hit& h ) -> float
			{
				const auto lethal = h.damage >= static_cast< float >( h.health );
				const auto prioritize_body = lethal || h.health < 40;
				const auto lethal_bonus = lethal ? 100000.0f : 0.0f;
				...
				return lethal_bonus + direct_bonus + center_bonus + h.damage * 20.0f +
					static_cast< float >( hitgroup_priority( h.hitbox_index, prioritize_body ) ) * 10.0f - h.fov;
			};
		...
		for ( auto& group : groups )
		{
			...
			for ( const auto idx : group.hit_indices )
			{
				...
				const auto prioritize_body = can_kill || h.health < 40;
				score += h.penetrated ? 0.0f : 250.0f;
				score += h.is_center ? 50.0f : 0.0f;
				score += static_cast< float >( hitgroup_priority( h.hitbox_index, prioritize_body ) ) * 50.0f;
				score -= h.fov * 0.1f;
		...
		for ( auto& cand : candidates )
		{
			for ( auto ri = 0; ri < cand.record_count; ++ri )
			{
				...
				for ( auto i = 0; i < hitbox_set.count; ++i )
				{
					...
					const auto trace = this->trace_taser_hit( eye, forward, shared_ctx.range * 0.95f, cand.pawn, local.pawn );
	...
	void rage::fire_gun( systems::input::usercmd* cmd, const target& tgt, bool was_forced, const math::vector3& shoot_eye, const systems::local::snapshot& local )
	{
		...
		if ( config.no_spread.value )
		{
			...
			const auto corrected = g_shared.find_spread_correction( aim_angle, stamp_tick, aim_punch );
		...
		for ( auto i = 0; i < history_size; ++i )
		{
			...
			{
				const auto shot_pitch = std::clamp( aim_angle.x - aim_punch.x, -89.0f, 89.0f );
				const auto shot_yaw = math::helpers::normalize_yaw( aim_angle.y - aim_punch.y );

				angles->set_x( shot_pitch );
				angles->set_y( shot_yaw );

				if ( config.no_spread.value )
				{
					angles->set_z( math::helpers::normalize_yaw( aim_angle.z ) );
				}
			}
		...
		{
			if ( const auto attack_step = systems::g_input.acquire_subtick_step( subtick_moves ) )
			{
				attack_step->set_button( cstypes::command_buttons::in_attack );
				attack_step->set_pressed( true );
				attack_step->set_when( 0.0f );
			}
		}
		...
		{
			command_aim.x = 89.0f;
			command_aim.y = math::helpers::normalize_yaw( punched_aim.y + 180.0f );
		}

		if ( const auto angles = base->mutable_viewangles( ) )
		{
			angles->set_x( std::clamp( command_aim.x, -89.0f, 89.0f ) );
			angles->set_y( math::helpers::normalize_yaw( command_aim.y ) );
			if ( config.no_spread.value )
			{
				angles->set_z( math::helpers::normalize_yaw( command_aim.z ) );
			}
		}
	...
	void rage::fire_melee( systems::input::usercmd* cmd, const target& tgt, const systems::local::snapshot& local )
	{
		...
		const auto melee_pitch = std::clamp( tgt.hit.aim_angle.x, -89.0f, 89.0f );
		const auto melee_yaw = math::helpers::normalize_yaw( tgt.hit.aim_angle.y );

		if ( entry )
		{
			if ( const auto angles = entry->mutable_view_angles( ) )
			{
				angles->set_x( melee_pitch );
				angles->set_y( melee_yaw );
			}
	...
	bool rage::should_stop_movement( const aim_context& ctx ) const
	{
		...
		if ( ctx.on_ground )
		{
			...
			return speed_2d * inaccuracy_move > inaccuracy_stand || speed_2d > ctx.accurate_threshold;
		}

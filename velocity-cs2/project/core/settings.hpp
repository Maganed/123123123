...
namespace settings {
	...
	struct combat
	{
		struct ragebot
		{
			...
			ragebot( )
			{
				...
				constexpr int default_min_dmg[ ]{ 10, 10, 20, 15, 30, 15 };

				for ( std::uint32_t i = 0; i < k_group_count; ++i )
				{
					this->groups[ i ].min_damage.value = default_min_dmg[ i ];
			...
			struct overlay
			{
				...
				struct box
				{
					...
					explicit box( const std::string& prefix )
						: enabled{ true, {}, "bounding box", prefix + " box" }
				...
				struct skeleton
				{
					...
					explicit skeleton( const std::string& prefix )
						: enabled{ true, {}, "skeleton", prefix + " skeleton" }
					{
	...
	struct misc
	{
		...
		struct widgets_cfg
		{
			...
			config::val<float> spectators_x{ -1.0f, "widgets", "spectators x" };
			config::val<float> spectators_y{ -1.0f, "widgets", "spectators y" };
		} m_widgets{};
	};
	...
	struct movement
	{
		...
		xui::setting jumpbug{ false, { 'C', xui::bind_mode::hold_on }, "jumpbug", "movement" };

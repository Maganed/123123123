...
namespace systems {
	...
	void input::update( )
	{
		...
		this->m_current_cmd = this->get_current_cmd( local.controller );
	}
	...
	void input::apply( )
	{
		...
		// Ensure analog forward and left deltas are applied across all commands
		...
		{
			const auto prev_fwd = memory::read<float>( movement_services + SCHEMA( "CPlayer_MovementServices", "m_flCmdForwardMove"_hash ) );
			const auto prev_left = memory::read<float>( movement_services + SCHEMA( "CPlayer_MovementServices", "m_flCmdLeftMove"_hash ) );
			const auto delta_fwd = base->forwardmove( ) - prev_fwd;
			const auto delta_left = base->leftmove( ) - prev_left;

			proto::subtick_move_step* initial_step = nullptr;
			for ( int i = 0; i < base->subtick_moves_size( ); ++i )
			{
				const auto s = base->mutable_subtick_moves( i );
				if ( s && s->when( ) == 0.0f )
				{
					initial_step = s;
					break;
			...
			if ( initial_step )
			{
				initial_step->set_analog_forward_delta( delta_fwd );
				initial_step->set_analog_left_delta( delta_left );
			}
			else if ( base->subtick_moves_size( ) > 0 )
			{
				if ( const auto first_step = base->mutable_subtick_moves( 0 ) )
				{
					first_step->set_analog_forward_delta( delta_fwd );
					first_step->set_analog_left_delta( delta_left );
				}
			}
			else
			{
				if ( const auto step = systems::g_input.acquire_subtick_step( base->mutable_subtick_moves( ) ) )
				{
					...
					step->set_when( 0.0f );
					step->set_analog_forward_delta( delta_fwd );
	...
	input::usercmd* input::get_current_cmd( std::uintptr_t local_controller ) const
	{
		...
		if ( !get_usercmd_base || !get_usercmd )
		{
			static int s_err1 = 0;
			if (++s_err1 % 300 == 1)
				logging::console::print( "[input] missing pattern get_usercmd_base={:#x} or get_usercmd={:#x}", get_usercmd_base, get_usercmd );
			return nullptr;
		}
		...
		if ( !usercmd_base )
		{
			static int s_err2 = 0;
			if (++s_err2 % 300 == 1)
				logging::console::print( "[input] get_usercmd_base({:#x}) returned 0", local_controller );
			return nullptr;
		}
		...
		if ( !sequence || *sequence <= 0 )
		{
			static int s_err3 = 0;
			if (++s_err3 % 300 == 1)
				logging::console::print( "[input] sequence from {:#x} is {}", usercmd_base + 0x5910, sequence ? *sequence : -1 );
			return nullptr;
		}

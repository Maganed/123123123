...
namespace hooks {
	...
	bool cheat::initialize () {
		...
		for (const auto& entry : feature_hooks) {
			if (!hooking::manager::create ({ entry })) {
				++unavailable_hooks;
				logging::console::print (xs ("[hooks]   [FAILED]  {:<32}"), entry.name);
			} else {
				logging::console::print (xs ("[hooks]   [OK]      {:<32} at {:#x}"), entry.name, entry.address);
			}
		}

		if (unavailable_hooks) {
			logging::console::print (
				xs ("[hooks] WARNING: feature hooks initialized with {} unavailable"),
				unavailable_hooks);
		} else {
			logging::console::print (xs ("[hooks] SUCCESS: All {} feature hooks successfully installed!"), std::size(feature_hooks));
		}
	...
	{
		// 1. Always let the game engine run its original CreateMove first to create the usercmd and handle input
		m_create_move.call<void>( thisptr, slot, active );

		const auto local = systems::g_local.get( );
		const bool has_cam = systems::g_view.has_camera( );

		// 2. Invalidate combat states if dead/uninitialized
		{
		...
		// 3. Safety checks: must have valid pawn/controller, be alive and have camera
		if ( !local.pawn || !local.controller || !local.is_alive || !has_cam )
		{
			return;
		}
		...
		// 4. Safely update input with the newly generated command for this tick
		systems::g_input.update( );
		{
			...
			diag::exception_scope exception_scope{ "create_move: pipeline" };
			if ( trace )
			{
	...
	{
		const auto local = systems::g_local.get( );
		if ( !local.is_valid( ) || !local.is_alive || !systems::g_input.get( ) )
		{
	...
	bool __fastcall cheat::get_transforms_for_hitbox_list( std::uintptr_t a1, std::uintptr_t a2, int* a3 )
	{
		...
		{
			static int s_fail_autowall = 0;
			if ( ++s_fail_autowall % 128 == 1 )
				logging::console::print( "[autowall] REJECT: count={}, shape_array={:#x}, bone_cache(a1+0x1c0)={:#x}, model(a1+0x1e0)={:#x}", count, shape_array, entity_bone_cache, model );
			return false;
		}
		...
		if ( !get_bone_index )
		{
			static int s_fail_gbi = 0;
			if ( ++s_fail_gbi % 128 == 1 )
				logging::console::print( "[autowall] REJECT: get_bone_index pattern missing" );
			return false;
		}
		...
		for ( auto i = 0; i < count; ++i )
		{
			...
			{
				static int s_fail_bone = 0;
				if ( ++s_fail_bone % 128 == 1 )
					logging::console::print( "[autowall] REJECT: stale bone cache (bone_index={}, cache={:#x})", bone_index, entity_bone_cache );
				return false;
			}
		}
